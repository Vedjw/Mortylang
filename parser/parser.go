...
const (
	_      int = iota // value is 0
	LOWEST            // value from 0 to 7 is assigned to each
	EQUALS
	LESSGREATER
	SUM
	PRODUCT
...
func (p *Parser) parseBoolean() ast.Expression {
	return &ast.Boolean{Token: p.curToken, Value: p.curTokenIs(token.TRUE)} // Clever
}
...
func (p *Parser) parseIfExpression() ast.Expression {
	...
	expression.Concequence = p.parseBlockStatement()
...
func (p *Parser) parseFuncionLiteral() ast.Expression {
	...
	if p.peekTokenIs(token.IDENT) {
		function.Name = &ast.Identifier{Token: p.peekToken, Value: p.peekToken.Literal}
		p.nextToken()
	}
